...
It mimics the appearance of multi-page websites that rely on server-side routing, but without the hard refresh. This means that the site doesn't have to do a full reload of all elements and components every time a user interacts with the site or goes to another page on the site. Thus, client-side routing is more efficient.
